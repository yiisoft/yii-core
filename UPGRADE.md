...
* Log targets (like `Yiisoft\Log\EmailTarget`) are now throwing `Yiisoft\Log\LogRuntimeException` in case log can not be properly exported.
...
* The signature of `Yiisoft\Cache\Cache::getOrSet()` has been adjusted to also accept a callable and not only `Closure`.
...
* `mail` component was renamed to `mailer`, `Yiisoft\Log\EmailTarget::$mail` was renamed to `Yiisoft\Log\EmailTarget::$mailer`.
