...
1. Criar um objeto  [[Yiisoft\Db\Query]] para representar diferentes partes de uma instrução SQL (ex. `SELECT`, `FROM`).
2. Executar um método (ex. `all()`) do objeto [[Yiisoft\Db\Query]] para recuperar dados do banco de dados.
...
> Observação: Geralmente, você trabalhará mais com o [[Yiisoft\Db\Query]] do que com o [[Yiisoft\Db\QueryBuilder]]. Este último é chamado pelo primeiro implicitamente quando você chama um dos métodos da query. O [[Yiisoft\Db\QueryBuilder]] é a classe responsável por gerar instruções SGDBs dependentes (ex. colocar aspas em nomes de tabela/coluna) a partir de objetos de query independentemente do SGDB.


## Construindo Queries <span id="building-queries"></span>

Para construir um objeto [[Yiisoft\Db\Query]], você pode chamar diferentes métodos de construção de query para especificar diferentes partes de uma instrução SQL. Os nomes destes métodos assemelha-se as palavras-chave SQL utilizados nas partes correspondentes da instrução SQL. Por exemplo, para especificar a parte da instrução SQL `FROM`, você deve chamar o método `from()`. Todos os métodos de construção de query retornam o próprio objeto query, que permite você encadear várias chamadas em conjunto. A seguir, descreveremos o uso de cada método de construção de query.


### [[Yiisoft\Db\Query::select()|select()]] <span id="select"></span>

O método [[Yiisoft\Db\Query::select()|select()]] especifica o fragmento de uma instrução SQL `SELECT`. Você pode especificar colunas para ser selecionado em um array ou uma string, como mostrado abaixo. Os nomes das colunas que estão sendo selecionadas serão automaticamente envolvidas entre aspas quando a instrução SQL está sendo gerada a partir do objeto query.
...
Se você não chamar o método [[Yiisoft\Db\Query::select()|select()]] na criação da query, o `*` será selecionado, o que significa selecionar *todas* as colunas.
...
A partir da versão 2.0.1, você também pode selecionar sub-queries. Você deve especificar cada sub-query na forma de um objeto [[Yiisoft\Db\Query]]. Por exemplo:
...
Para utilizar a cláusula `distinct`, você pode chamar [[Yiisoft\Db\Query::distinct()|distinct()]], como a seguir:
...
Você pode chamar [[Yiisoft\Db\Query::addSelect()|addSelect()]] para selecionar colunas adicionais. Por exemplo:
...
O método [[Yiisoft\Db\Query::from()|from()]] especifica o fragmento de uma instrução SQL `FROM`. Por exemplo:
...
Além de nome de tabelas, você também pode selecionar a partir de sub-queries especificando-o um objeto [[Yiisoft\Db\Query]].
Por exemplo:
...
O método [[Yiisoft\Db\Query::where()|where()]] especifica o fragmento de uma instrução SQL `WHERE`. Você pode usar um dos três formatos para especificar uma condição `WHERE`:
...
Ao usar parâmetro, você pode chamar [[Yiisoft\Db\Query::params()|params()]] ou [[Yiisoft\Db\Query::addParams()|addParams()]] para especificar os parâmetros separadamente.
...
- `exists`: requer um operador que deve ser uma instância de [[Yiisoft\Db\Query]] representando a sub-query. Isto criará uma expressão `EXISTS (sub-query)`.
...
Você pode usar [[Yiisoft\Db\Query::andWhere()|andWhere()]] ou [[Yiisoft\Db\Query::orWhere()|orWhere()]] para acrescentar condições adicionais a uma condição já existente. Você pode chamá-los várias vezes para acrescentar várias condições separadamente. Por exemplo:
...
A única diferença entre  [[Yiisoft\Db\Query::filterWhere()|filterWhere()]] e [[Yiisoft\Db\Query::where()|where()]] é que o primeiro irá ignorar valores vazios fornecidos na condição no [formato hash](#hash-format). Então se `$email` for vazio e `$username` não, o código acima resultará um SQL como: `...WHERE username=:username`.

> Observação: Um valor é considerado vazio se ele for `null`, um array vazio, uma string vazia ou uma string que consiste em apenas espaços em branco. Assim como [[Yiisoft\Db\Query::andWhere()|andWhere()]] e [[Yiisoft\Db\Query::orWhere()|orWhere()]], você pode usar [[Yiisoft\Db\Query::andFilterWhere()|andFilterWhere()]] e [[Yiisoft\Db\Query::orFilterWhere()|orFilterWhere()]] para inserir condições de filtro adicionais.


### [[Yiisoft\Db\Query::orderBy()|orderBy()]] <span id="order-by"></span>

O método  [[Yiisoft\Db\Query::orderBy()|orderBy()]] especifica o fragmento de uma instrução SQL `ORDER BY`. Por exemplo:
...
Você pode chamar [[Yiisoft\Db\Query::addOrderBy()|addOrderBy()]] para incluir colunas adicionais para o fragmento `ORDER BY`. Por exemplo:
...
O método [[Yiisoft\Db\Query::groupBy()|groupBy()]] especifica o fragmento de uma instrução SQL `GROUP BY`. Por exemplo:
...
Você pode chamar [[Yiisoft\Db\Query::addGroupBy()|addGroupBy()]] para incluir colunas adicionais ao fragmento `GROUP BY`. Por exemplo:
...
O método [[Yiisoft\Db\Query::having()|having()]] especifica o fragmento de uma instrução SQL `HAVING`. Este método recebe uma condição que pode ser especificada da mesma forma como é feito para o [where()](#where). Por exemplo:
...
Você pode chamar [[Yiisoft\Db\Query::andHaving()|andHaving()]] ou [[Yiisoft\Db\Query::orHaving()|orHaving()]] para incluir uma condição adicional para o fragmento `HAVING`. Por exemplo:
...
### [[Yiisoft\Db\Query::limit()|limit()]] e [[Yiisoft\Db\Query::offset()|offset()]] <span id="limit-offset"></span>

Os métodos  [[Yiisoft\Db\Query::limit()|limit()]] e [[Yiisoft\Db\Query::offset()|offset()]] especificam os fragmentos de uma instrução SQL `LIMIT` e `OFFSET`. Por exemplo:
...
O método [[Yiisoft\Db\Query::join()|join()]] especifica o fragmento de uma instrução SQL `JOIN`. Por exemplo:
...
O método [[Yiisoft\Db\Query::join()|join()]] recebe quatro parâmetros:
...
Para unir múltiplas tabelas, chame os métodos join acima multiplas vezes, uma para cada tabela. Além de unir tabelas, você também pode unir sub-queries. Para fazê-lo, especifique a sub-queries a ser unida como um objeto [[Yiisoft\Db\Query]]. Por exemplo:
...
O método [[Yiisoft\Db\Query::union()|union()]] especifica o fragmento de uma instrução SQL `UNION`. Por exemplo:
...
Você  pode chamar [[Yiisoft\Db\Query::union()|union()]] múltiplas vezes para acrescentar mais fragmentos `UNION`. 


## Métodos Query <span id="query-methods"></span>

[[Yiisoft\Db\Query]] fornece um conjunto de métodos para diferentes propósitos da consulta:

- [[Yiisoft\Db\Query::all()|all()]]: retorna um array de linhas sendo cada linha um array de pares nome-valor.
- [[Yiisoft\Db\Query::one()|one()]]: retorna a primeira linha do resultado.
- [[Yiisoft\Db\Query::column()|column()]]: retorna a primeira coluna do resultado.
- [[Yiisoft\Db\Query::scalar()|scalar()]]: retorna um valor escalar localizado na primeira linha e coluna do primeiro resultado.
- [[Yiisoft\Db\Query::exists()|exists()]]: retorna um valor que indica se a consulta contém qualquer resultado.
- [[Yiisoft\Db\Query::count()|count()]]: retorna a quantidade de resultados da query.
- Outros métodos de agregação da query, incluindo [[Yiisoft\Db\Query::sum()|sum($q)]], [[Yiisoft\Db\Query::average()|average($q)]], [[Yiisoft\Db\Query::max()|max($q)]], [[Yiisoft\Db\Query::min()|min($q)]]. O parâmetro `$q` é obrigatório para estes métodos e pode ser um nome de uma coluna ou expressão DB. Por exemplo:
...
Quando você chamar um método de [[Yiisoft\Db\Query]], ele na verdade faz o seguinte trabalho por baixo dos panos:

* Chama [[Yiisoft\Db\QueryBuilder]] para gerar uma instrução SQL com base na atual construção de [[Yiisoft\Db\Query]];
* Cria um objeto [[Yiisoft\Db\Command]] com a instrução SQL gerada;
* Chama um método query (ex. `queryAll()`) do [[Yiisoft\Db\Command]] para executar a instrução SQL e retornar os dados.

Algumas vezes, você pode querer examinar ou usar a instrução SQL construído a partir de um objeto [[Yiisoft\Db\Query]]. Você pode atingir este objetivo com o seguinte código: 
...
Quando você chama [[Yiisoft\Db\Query::all()|all()]], será retornado um array de linhas que são indexadas por inteiros consecutivos. Algumas vezes você pode querer indexa-los de forma diferente, tal como indexar por uma coluna ou valor de expressão em particular. Você pode atingir este objetivo chamando [[Yiisoft\Db\Query::indexBy()|indexBy()]] antes de [[Yiisoft\Db\Query::all()|all()]]. Por exemplo:
...
Para indexar através de valores de expressão, passe uma função anônima para o método [[Yiisoft\Db\Query::indexBy()|indexBy()]]:
...
O método [[Yiisoft\Db\Query::batch()]] and [[Yiisoft\Db\Query::each()]] retorna um objeto [[Yiisoft\Db\BatchQueryResult]] que implementa a interface `Iterator` e, assim, pode ser utilizado na construção do `foreach`. Durante a primeira iteração, uma consulta SQL é feita à base de dados. Os dados são, então, baixados em lotes nas iterações restantes. Por padrão, o tamanho do batch é 100, significando 100 linhas de dados que serão baixados a cada batch. Você pode mudar o tamanho do batch passando o primeiro parâmetro para os métodos `batch()` ou `each()`.

Em comparação com o [[Yiisoft\Db\Query::all()]], o batch query somente carrega 100 linhas de dados na memória a cada vez. Se você processar os dados e, em seguida, descartá-lo imediatamente, o batch query pode ajudar a reduzir o uso de memória. Se você especificar o resultado da query a ser indexado por alguma coluna via [[Yiisoft\Db\Query::indexBy()]], o batch query ainda vai manter o índice adequado. Por exemplo:
