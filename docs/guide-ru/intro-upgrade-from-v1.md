...
`CDbCriteria`, и `CDbCommandBuilder`. В Yii 2.0 запрос к БД представлен в рамках объекта [[Yiisoft\Db\Query|Query]],
который может быть превращён в SQL выражение с помощью [[Yiisoft\Db\QueryBuilder|QueryBuilder]]. Например,
...
Класс `CDbCriteria` версии 1.1 был заменен [[Yiisoft\Db\ActiveQuery]]. Этот класс наследуется от [[Yiisoft\Db\Query]] и таким
...
Для объявления связи следует просто объявить геттер, который возвращает объект [[Yiisoft\Db\ActiveQuery|ActiveQuery]].
...
Вместо того, чтобы при выборке большого количества записей возвращать объекты [[Yiisoft\Db\ActiveRecord|ActiveRecord]], вы
можете использовать в построении запроса метод [[Yiisoft\Db\ActiveQuery::asArray()|asArray()]]. Это заставит вернуть
