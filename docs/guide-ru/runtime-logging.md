...
> Info: методы логирования, описанные выше являются, на самом деле, ярлыками для метода [[Yiisoft\Log\Logger::log()|log()]] [[Yiisoft\Log\Logger|объекта логгера]], который доступен как синглтон `Yii::getLogger()`.
При определенном количестве записанных сообщений или завершении приложения, объект логгера вызывает [[Yiisoft\Log\Dispatcher|message dispatcher]] для отправки записанных сообщений зарегистрированным [целям логов](#log-targets).


## Цели логов <span id="log-targets"></span>

Цель логов - это экземпляр класса [[Yiisoft\Log\Target]] или класса, унаследованного от него. Цель фильтрует сообщения логов по уровню важности и категории, а затем выгружает их в соответствующее хранилище. Например, [[Yiisoft\Log\DbTarget|database target]] выгружает отфильтрованные сообщения логов в таблицу базы данных, а [[Yiisoft\Log\EmailTarget|email target]] отправляет сообщения логов на заданные адреса email.
...
В приведенном выше коде в свойстве [[Yiisoft\Log\Dispatcher::targets]] зарегистрированы две цели логов:

* первая цель выбирает ошибки и предупреждения и сохраняет их в базу данных;
* вторая цель выбирает ошибки с категорией, имя которой начинается с `Yiisoft\Db\` и шлет сразу на два адреса email `admin@example.com` и `developer@example.com`.
...
* [[Yiisoft\Log\DbTarget]]: сохраняет сообщения логов в таблицу базы данных.
* [[Yiisoft\Log\EmailTarget]]: шлет сообщения логов на заранее указанный email.
* [[Yiisoft\Log\FileTarget]]: сохраняет сообщения логов в файлы.
* [[Yiisoft\Log\SyslogTarget]]: сохраняет сообщения логов в системный лог используя функцию PHP `syslog()`.
...
Для каждой цели можно настроить свойства [[Yiisoft\Log\Target::levels|levels]] и [[Yiisoft\Log\Target::categories|categories]], которые указывают уровни важности и категории сообщений логов, которые цель должна обрабатывать.

Свойство [[Yiisoft\Log\Target::levels|levels]] принимает массив, содержащий одно или несколько следующих значений:
...
Если свойство [[Yiisoft\Log\Target::levels|levels]] не задано, цель логов будет обрабатывать сообщения с *любым* уровнем важности.

Свойство [[Yiisoft\Log\Target::categories|categories]] принимает массив, содержащий имена категорий или шаблоны.
Цель будет обрабатывать только те сообщения, категория которых совпадает с одним из значений или шаблонов этого массива. Шаблон категории должен состоять из префикса имени категории и звездочки `*` на конце. Имя категории совпадает с шаблоном, если оно начинается с префикса шаблона. Например, `Yiisoft\Db\Command::execute` и `Yiisoft\Db\Command::query` используются в качестве имен категорий сообщений, записанных в классе [[Yiisoft\Db\Command]]. Оба они совпадают с шаблоном `Yiisoft\Db\*`.

Если свойство [[Yiisoft\Log\Target::categories|categories]] не задано, цель будет обрабатывать сообщения любой категории.

Кроме списка включаемый категорий, заданного свойством [[Yiisoft\Log\Target::categories|categories]], при помощи свойства [[Yiisoft\Log\Target::except|except]] возможно задать список исключаемых категорий. Если категория сообщения совпадает со значением или шаблоном из списка исключаемых категорий, такое сообщение не будет обработано.

В следующем примере показан вариант конфигурации цели логов, которая должна обрабатывать только сообщения об ошибках и предупреждениях в категориях `Yiisoft\Db\*` и `yii\web\HttpException:*`, за исключением `yii\web\HttpException:404`.
...
Цели логов выгружают отфильтрованные сообщения в определенном формате. Например, цель класса [[Yiisoft\Log\FileTarget]] сохранит сообщение следующего формата в файле `runtime/log/app.log`:

```
2014-10-04 18:10:15 [::1][][-][trace][yii\base\Module::getModule] Loading module: debug
```

По умолчанию сообщения логов форматируются методом [[Yiisoft\Log\Target::formatMessage()]]:

```
Временная метка [IP адрес][ID пользователя][ID сессии][Уровень важности][Категория] Текст сообщения
```

Этот формат может быть изменен при помощи свойства [[Yiisoft\Log\Target::prefix]], которое получает анонимную функцию, возвращающую нужный префикс сообщения. Например, следующий код позволяет настроить вывод идентификатор текущего пользователя в качестве префикса для всех сообщений.
...
`$_SESSION` и `$_SERVER`. Эта возможность настраивается при помощи свойства [[Yiisoft\Log\Target::logVars]], содержащего массив имен переменных, которые необходимо включить в лог. Например, следующий код позволяет настроить цель логов так, чтобы к сообщениям присоединялось только содержимое переменной `$_SERVER`.
...
Для определения собственного алгоритма подключения общей информации, следует переопределить метод [[Yiisoft\Log\Target::getContextMessage()]].


### Уровень отслеживания выполнения кода <span id="trace-level"></span>

В процессе разработки, часто бывает необходимость видеть источники сообщений. Для этого нужно использовать свойство [[Yiisoft\Log\Dispatcher::traceLevel|traceLevel]] компонента `log`. Например,
...
Как упоминалось выше, сообщения логов обрабатываются в массиве [[Yiisoft\Log\Logger|объектом логгера]]. Для ограничения объема памяти, занятого этим массивом, при накоплении определенного числа сообщений, логгер передает их на обработку [целям логов](#log-targets). Максимальное количество сообщений определяется свойством [[Yiisoft\Log\Dispatcher::flushInterval|flushInterval]] компонента `log`:
...
После передачи сообщений [[Yiisoft\Log\Logger|объектом логгера]] в [цели логов](#log-targets), сообщения не выгружаются немедленно. Вместо этого, выгрузка сообщений происходит когда цель логов накопит определенное количество фильтрованных сообщений. Максимальное количество сообщений определяется свойством [[Yiisoft\Log\Target::exportInterval|exportInterval]] [цели логов](#log-targets). Например,
...
Свойство [[Yiisoft\Log\Target::enabled|enabled]] отвечает за включение или отключение цели логов. Возможно управлением этим свойством как в конфигурации приложения, так и при помощи следующего PHP-кода:
...
Создание новой цели логов не является сложной задачей. В общем случае нужно реализовать метод [[Yiisoft\Log\Target::export()]], выгружающий массив [[Yiisoft\Log\Target::messages]] в место хранения логов. Возможно использование метода [[Yiisoft\Log\Target::formatMessage()]] для форматирования сообщения. Детали реализации можно подсмотреть в исходном коде любого из классов целей логов, включенных в состав Yii.


## Профилирование производительности <span id="performance-profiling"></span>

Профилирование производительности - это специальный тип сообщений логов, используемый для измерения времени выполнения определенных участков кода и определения проблем производительности. Например, класс [[Yiisoft\Db\Command]] использует профилирование производительности для определения времени исполнения каждого запроса базы данных.
