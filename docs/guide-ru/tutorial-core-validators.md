...
- `filter`: дополнительный фильтр, который будет добавлен к запросу в базу данных для проверки на существование значения.
  ...
  значений запроса: [[Yiisoft\Db\Query::where()]]), или анонимная функция с сигнатурой `function ($query)`,
  где `$query` - это [[Yiisoft\Db\Query|Query]] объект, который вы можете модифицировать в функции.
...
- `filter`: дополнительный фильтр, который можно присоединить к запросу в БД, чтобы использовать его при
  ...
  (см. [[Yiisoft\Db\Query::where()]] о формате условий в запросе), или анонимная функция вида `function ($query)`,
  где `$query` это объект [[Yiisoft\Db\Query|Query]], который вы можете изменить в функции.
