...
повинні бути успадковані від класу [[Yiisoft\Yii\Console\Controller]], аналогічного `CConsoleCommand` у версії 1.1.
...
[[Yiisoft\Yii\Console\Controller::options()]].
...
`CDbCriteria` та `CDbCommandBuilder`. У Yii 2.0 запит до БД представлений в рамках обʼєкта [[Yiisoft\Db\Query|Query]],
який може бути перетворений у SQL-вираз за допомогою [[Yiisoft\Db\QueryBuilder|QueryBuilder]]. Наприклад,
...
Клас `CDbCriteria` у версії 1.1 був замінений на [[Yiisoft\Db\ActiveQuery]] у Yii 2.0. Цей клас успадковується від
[[Yiisoft\Db\Query]] і таким чином отримує всі методи, які необхідні для побудови запиту. Для побудови запиту вам слід
викликати метод  [[Yiisoft\Db\ActiveRecord::find()]]:
...
Для оголошення звʼязку слід просто призначити геттер, який повертає обʼєкт [[Yiisoft\Db\ActiveQuery|ActiveQuery]].
...
Замість того, щоб повертати обʼєкти [[Yiisoft\Db\ActiveRecord|ActiveRecord]], ви можете використовувати метод
[[Yiisoft\Db\ActiveQuery::asArray()|asArray()]] при побудові запиту, для вибірки великої кількості записів.
...
[[Yiisoft\Db\ActiveRecord::instantiate()]]. Перевизначення може не знадобитися, якщо параметри, передані в конструктор
