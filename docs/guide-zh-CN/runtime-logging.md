...
> Note: 上面所描述的日志方法实际上是 [[Yiisoft\Log\Logger|logger object]] 对象（一个通过表达式 `Yii::getLogger()` 可访问的单例）
的方法 [[Yiisoft\Log\Logger::log()|log()]] 的一个快捷方式。当足够的消息被记录或者当应用结束时，
日志对象将会调用一个 [[Yiisoft\Log\Dispatcher|message dispatcher]]
调度对象将已经记录的日志消息发送到已注册的 [log targets](#log-targets) 目标中。


## 日志目标 <span id="log-targets"></span>

一个日志目标是一个 [[Yiisoft\Log\Target]] 类或者它的子类的实例。
它将通过他们的严重层级和类别来过滤日志消息，然后将它们导出到一些媒介中。
例如，一个 [[Yiisoft\Log\DbTarget|database target]] 目标导出已经过滤的日志消息到一个数据的表里面，
而一个 [[Yiisoft\Log\EmailTarget|email target]]目标将日志消息导出到指定的邮箱地址里。
...
* [[Yiisoft\Log\DbTarget]]：在数据库表里存储日志消息。
* [[Yiisoft\Log\EmailTarget]]：发送日志消息到预先指定的邮箱地址。
* [[Yiisoft\Log\FileTarget]]：保存日志消息到文件中.
* [[Yiisoft\Log\SyslogTarget]]：通过调用PHP函数 `syslog()` 将日志消息保存到系统日志里。
...
对于每一个日志目标，你可以配置它的 [[Yiisoft\Log\Target::levels|levels]] 和 
[[Yiisoft\Log\Target::categories|categories]] 属性来指定哪个消息的严重程度和分类目标应该处理。

[[Yiisoft\Log\Target::levels|levels]] 属性是由一个或者若干个以下值组成的数组：
...
如果你没有指定 [[Yiisoft\Log\Target::levels|levels]] 的属性，
那就意味着目标将处理 *任何* 严重程度的消息。

[[Yiisoft\Log\Target::categories|categories]] 属性是一个包含消息分类名称或者模式的数组。
...
除了通过 [[Yiisoft\Log\Target::categories|categories]] 属性设置白名单分类，你也可以通过 [[Yiisoft\Log\Target::except|except]]
...
假如你安装一个 [[Yiisoft\Log\FileTarget]] 类的日志目标，
...
默认情况下，日志消息将被格式化，格式化的方式遵循 [[Yiisoft\Log\Target::formatMessage()]]：

```
Timestamp [IP address][User ID][Session ID][Severity Level][Category] Message Text
```

你可以通过配置 [[Yiisoft\Log\Target::prefix]] 的属性来自定义格式，这个属性是一个PHP可调用体返回的自定义消息前缀。
...
你可以通过配置 [[Yiisoft\Log\Target::logVars]] 属性适应这个行为，
...
你可以重写 [[Yiisoft\Log\Target::getContextMessage()]] 方法。


### 消息跟踪级别 <span id="trace-level"></span>

在开发的时候，通常希望看到每个日志消息来自哪里。这个是能够被实现的，通过配置 `log` 组件的 [[Yiisoft\Log\Dispatcher::traceLevel|traceLevel]] 属性，
就像下面这样：
...
上面的应用配置设置了 [[Yiisoft\Log\Dispatcher::traceLevel|traceLevel]] 的层级，假如 `YII_DEBUG` 开启则是3，否则是0。
...
如上所述，通过 [[Yiisoft\Log\Logger|logger object]] 对象，日志消息被保存在一个数组里。
为了这个数组的内存消耗，当数组积累了一定数量的日志消息，
日志对象每次都将刷新被记录的消息到 [log targets](#log-targets) 中。
你可以通过配置 `log` 组件的 [[Yiisoft\Log\Dispatcher::flushInterval|flushInterval]] 属性来自定义数量：
...
当 [[Yiisoft\Log\Logger|logger object]] 对象刷新日志消息到 [log targets](#log-targets) 的时候，它们并
不能立即获取导出的消息。相反，消息导出仅仅在一个日志目标累积了一定数量的过滤消息的时候才会发生。你可以通过配置
个别的 [log targets](#log-targets) 的 [[Yiisoft\Log\Target::exportInterval|exportInterval]] 属性来
自定义这个数量，就像下面这样：
...
你应该设置 [[Yiisoft\Log\Dispatcher::flushInterval|flushInterval]]
和 [[Yiisoft\Log\Target::exportInterval|exportInterval]] 都为1，
就像下面这样：
...
你可以通过配置 [[Yiisoft\Log\Target::enabled|enabled]] 属性来开启或者禁用日志目标。
...
创建一个新的日志目标类非常地简单。你主要需要实现 [[Yiisoft\Log\Target::export()]] 
方法来发送 [[Yiisoft\Log\Target::messages]] 数组的
内容到一个指定的媒体中。你可以调用 [[Yiisoft\Log\Target::formatMessage()]] 方法去格式化每个消息。
